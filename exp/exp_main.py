            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if any(substr in self.args.model for substr in {'CycleNet', 'PRNet', 'PRNet_no_CA', 'PRNet_no_Gate', 'PRNet_no_Cycle', 'PRNet_CBAM_style','PRNet_GatingVisualization','PRNet_Sigmoid'}):
                    ...
                    if any(substr in self.args.model for substr in {'CycleNet', 'PRNet'}):
                        outputs = self.model(batch_x, batch_cycle)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if any(substr in self.args.model for substr in {'CycleNet', 'PRNet', 'PRNet_no_CA', 'PRNet_no_Gate', 'PRNet_no_Cycle', 'PRNet_CBAM_style'}):
                            outputs = self.model(batch_x, batch_cycle)
                        elif any(substr in self.args.model for substr in
                                 {'Linear', 'MLP', 'SegRNN', 'TST'}):
                            outputs = self.model(batch_x)
